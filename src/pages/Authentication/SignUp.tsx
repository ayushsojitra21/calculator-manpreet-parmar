...
const SignUp: React.FC = () => {
  ...
  const handleGoogleLoginSuccess = async (response: CredentialResponse) => {
    try {
      ...
      const ftoken = await requestToken();

      const { data } = await axios.post(
        'http://localhost:3005/api/v1/users/social/login',
        {
  ...
  const requestToken = async (): Promise<string> => {
    // Replace this with the actual implementation to get a token
  ...
  const responseMessage = async (response) => {
    ...
    console.log('Sssaaaa', decoded);

    const ftoken = await requestToken().then((result) => {
    ...
    axios
      .post('http://localhost:3005/api/v1/users/signUp', {
        first_name: decoded.given_name,
        last_name: decoded.family_name,
        email: decoded.email,
        social_id: decoded.sub,
